...
export default function ReorderSuggestionsPage() {
	...
	return (
		<div className="px-4 sm:px-6 lg:px-7 pt-5 lg:pt-6 pb-20 max-w-[1400px]">
			{/* Title row */}
			<div className="flex items-end gap-3.5 mb-5 flex-wrap">
				...
				{suggestions && (
					<button
						onClick={compute}
						disabled={busy}
						className="h-10 sm:h-9 flex-1 sm:flex-none px-3.5 rounded border border-line-2 bg-surface text-body-3 font-bold text-[12.5px] cursor-pointer disabled:opacity-50 hover:border-brand-300 hover:text-brand-600 transition-all duration-200 ease-smooth flex items-center justify-center gap-1.5">
				...
				<button
					...
					className="h-10 sm:h-9 flex-1 sm:flex-none px-[18px] rounded border border-brand bg-brand hover:bg-brand-600 text-white font-bold text-[13px] cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2 transition-all duration-200 ease-smooth">
					{busy && (
			...
			{suggestions && suggestions.length > 0 && (
				<div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 mb-4 px-4 sm:px-[18px] py-3 bg-surface border border-line rounded max-w-[760px]">
			...
			<div className="bg-surface border border-line rounded overflow-hidden">
				<div
					className="hidden lg:grid px-5 py-3 bg-surface-2 border-b border-line text-[10.5px] font-black text-muted tracking-[.06em]"
			...
			{toasts.length > 0 && (
				<div className="fixed bottom-3 sm:bottom-6 left-3 right-3 sm:left-auto sm:right-6 z-[100] flex flex-col gap-2.5 items-stretch sm:items-end pointer-events-none max-h-[calc(100vh-100px)] overflow-y-auto">
					{toasts.map((t) => (
 ...
 *
 * The table puts current and proposed side by side across four columns, which
 * needs width this layout does not have. Stacked, each figure keeps its label
 * and the two decisions become full-width buttons — an approve/reject pair at
 * 34px square is a poor target for a thumb, and getting it wrong here means
 * approving a reorder point you meant to reject.
 */
function SuggestionCard({ suggestion, index, onApprove, onReject }) {
	const s = suggestion;

	const line = (label, prev, next) => {
		const to = next == null ? prev : next;
		const same = to === prev;
		const up = to > prev;
		return (
			<div className="flex items-center justify-between gap-3 py-1.5">
				...
				<span className="flex items-center gap-2">
					<span className="num text-[13px] text-muted-2">{prev}</span>
					<svg width="14" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" className={same ? 'text-line-2' : 'text-brand-300'}>
						<path d="M5 12h14M13 6l6 6-6 6" strokeLinecap="round" strokeLinejoin="round" />
					</svg>
					<span
						className={`num inline-flex items-center px-2 py-[3px] rounded text-[14px] font-black ${
							same
								? 'text-muted-2'
								: 'bg-brand-50 border border-brand-100 text-brand-700'
						}`}>
	...
	return (
		<div
			...
			<div className="font-black text-[14.5px] text-heading">{s.item_name}</div>

			<div className="flex items-center gap-2 mt-1.5 min-w-0">
				<ConfidenceBar level={s.confidence} />
				<span className="text-[11.5px] text-muted-2 truncate" title={s.reason}>
					{subLineFor(s)}
				</span>
			</div>

			<div className="mt-2.5 border-t border-line-4 pt-1.5">
				{line('Reorder point', s.current_rop, s.proposed_rop)}
				{line('Max capacity', s.current_max, s.proposed_max)}
			</div>

			<div className="flex items-center gap-2 mt-3">
				<button
					onClick={onApprove}
					className="flex-1 h-10 rounded border border-ok-border bg-ok-bg text-ok font-bold text-[13px] cursor-pointer flex items-center justify-center gap-1.5">
					<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.8">
						<path d="M20 6L9 17l-5-5" strokeLinecap="round" strokeLinejoin="round" />
					</svg>
					Approve
				...
				<button
					onClick={onReject}
					className="flex-1 h-10 rounded border border-line-2 bg-surface text-body-3 font-bold text-[13px] cursor-pointer flex items-center justify-center gap-1.5">
					<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.6" strokeLinecap="round">
						<path d="M6 6l12 12M18 6L6 18" />
					</svg>
					Reject
